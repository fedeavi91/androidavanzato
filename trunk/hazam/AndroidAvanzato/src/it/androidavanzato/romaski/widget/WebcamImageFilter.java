...
import it.androidavanzato.romaski.fs.ImageCache.OnLoadFilter;
import it.androidavanzato.romaski.fs.ImageCache.OnSaveFilter;
...
public class WebcamImageFilter implements OnSaveFilter, OnLoadFilter {
	...
	public WebcamImageFilter(Context ctx) {
	...
	public void saveResolve(String name, InputStream src, OutputStream out) {
		...
		left = left < 0 ? 0 : left;
		top = top < 0 ? 0 : top;
		synchronized (WebcamImageFilter.class) {
			m.setScale(sx, sy);
